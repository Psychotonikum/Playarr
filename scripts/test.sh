...
FILES=( "Playarr.Api.Test.dll" "Playarr.Automation.Test.dll" "Playarr.Common.Test.dll" "Playarr.Core.Test.dll" "Playarr.Host.Test.dll" "Playarr.Integration.Test.dll" "Playarr.Libraries.Test.dll" "Playarr.Mono.Test.dll" "Playarr.Update.Test.dll" "Playarr.Windows.Test.dll" )
ASSMEBLIES=""
...
export PLAYARR_TESTS_LOG_OUTPUT="File"
...
if [ "$PLATFORM" = "Windows" ]; then
  mkdir -p "$ProgramData/Playarr"
  WHERE="$WHERE&Category!=LINUX"
elif [ "$PLATFORM" = "Linux" ]; then
  mkdir -p ~/.config/Playarr
  WHERE="$WHERE&Category!=WINDOWS"
elif  [ "$PLATFORM" = "Mac" ]; then
  mkdir -p ~/Library/Application\ Support/Playarr
  WHERE="$WHERE&Category!=WINDOWS"
else
