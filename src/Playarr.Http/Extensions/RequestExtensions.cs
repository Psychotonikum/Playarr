...
namespace Playarr.Http.Extensions
{
    public static class RequestExtensions
    {
        ...
        {
            "game.sortname", // Workaround authors table properties not being added on isValidSortKey call
            "rom.title", // Deprecated
            "rom.airDateUtc", // Deprecated
            "rom.language", // Deprecated
            "timeleft",
        ...
        {
            if (request.Headers.TryGetValue("X-Playarr-Client", out var source))
            {
                return source;
            }

            return Playarr.Common.Http.UserAgentParser.ParseSource(request.Headers["User-Agent"]);
        }
