...
namespace Playarr.Host.AccessControl
{
    ...
    public class FirewallAdapter : IFirewallAdapter
    {
        ...
        public void MakeAccessible()
        {
            ...
            {
                if (!IsPlayarrPortOpen(_configFileProvider.Port))
                {
                    _logger.Debug("Opening Port for Playarr: {0}", _configFileProvider.Port);
                    OpenFirewallPort(_configFileProvider.Port);
                }

                if (_configFileProvider.EnableSsl && !IsPlayarrPortOpen(_configFileProvider.SslPort))
                {
                    _logger.Debug("Opening SSL Port for Playarr: {0}", _configFileProvider.SslPort);
                    OpenFirewallPort(_configFileProvider.SslPort);
                }
            }
        }

        private bool IsPlayarrPortOpen(int port)
        {
            try
            {
        ...
        private void OpenFirewallPort(int portNumber)
        {
            ...
            {
                _logger.Warn(ex, "Failed to open port in firewall for Playarr " + portNumber);
            }
        }
