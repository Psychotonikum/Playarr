    ...
    public class UpdateServiceFixture : CoreTest<InstallUpdateService>
    {
        ...
        public void Setup()
        {
            if (OsInfo.IsLinux)
            {
                ...
                {
                    FileName = "Playarr.develop.2.0.0.0.tar.gz",
                    Url = "http://download.playarr.tv/v2/develop/mono/Playarr.develop.tar.gz",
            ...
            else
            {
                _updatePackage = new UpdatePackage
                {
                    FileName = "Playarr.develop.2.0.0.0.zip",
                    Url = "http://download.playarr.tv/v2/develop/windows/Playarr.develop.zip",
                    Version = new Version("2.0.0.0")
                };
            }

            Mocker.GetMock<IAppFolderInfo>().SetupGet(c => c.TempFolder).Returns(TempFolder);
            Mocker.GetMock<IAppFolderInfo>().SetupGet(c => c.StartUpFolder).Returns(@"C:\Playarr".AsOsAgnostic);
            Mocker.GetMock<IAppFolderInfo>().SetupGet(c => c.AppDataFolder).Returns(@"C:\ProgramData\Playarr".AsOsAgnostic);
            ...
            Mocker.GetMock<IRuntimeInfo>().Setup(c => c.ExecutingApplication).Returns(@"C:\Test\Playarr.exe");
            ...
            Mocker.GetMock<IDiskProvider>()
                  .Setup(v => v.FileExists(It.Is<string>(s => s.EndsWith("Playarr.Update".ProcessNameToExe()))))
                  .Returns(true);
        ...
        {
            const string scriptPath = "/tmp/playarr/update.sh";
        ...
        public void Should_download_and_extract_to_temp_folder()
        {
            ...
            updateSubFolder.GetDirectories("Playarr").Should().HaveCount(1);
        ...
        {
            Mocker.GetMock<IAppFolderInfo>().SetupGet(c => c.StartUpFolder).Returns(@"C:\Playarr".AsOsAgnostic);
            Mocker.GetMock<IAppFolderInfo>().SetupGet(c => c.AppDataFolder).Returns(@"C:\Playarr\AppData".AsOsAgnostic);
        ...
        {
            Mocker.GetMock<IAppFolderInfo>().SetupGet(c => c.StartUpFolder).Returns(@"C:\Playarr".AsOsAgnostic);
            Mocker.GetMock<IAppFolderInfo>().SetupGet(c => c.AppDataFolder).Returns(@"C:\Playarr".AsOsAgnostic);
