...
namespace Playarr.Core.Test.DecisionEngineTests
{
    ...
    public class ReleaseRestrictionsSpecificationFixture : CoreTest<ReleaseRestrictionsSpecification>
    {
        ...
        {
            _remoteRom.Release.Title = "[ www.Speed.cd ] -Whose.Line.is.it.Anyway.US.S10E24.720p.HDTV.x264-BAJSKORV";
        ...
        public void should_match_perl_regex(string pattern, bool expected)
        {
            ...
            Subject.IsSatisfiedBy(_remoteRom, new()).Accepted.Should().Be(expected);
        }
    }
}
