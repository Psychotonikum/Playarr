...
using Playarr.Core.Download.Clients.Sabnzbd;
using Playarr.Core.Download.Clients.Sabnzbd.Responses;
using Playarr.Core.RemotePathMappings;
using Playarr.Core.Games;
using Playarr.Core.Validation;
using Playarr.Test.Common;

namespace Playarr.Core.Test.Download.DownloadClientTests.SabnzbdTests
{
    [TestFixture]
    public class SabnzbdFixture : DownloadClientFixtureBase<Sabnzbd>
    {
        ...
        public async Task Download_should_use_sabRecentTvPriority_when_recentEpisode_is_true()
        {
            Mocker.GetMock<ISabnzbdProxy>()
                    ...
                    .Returns(new SabnzbdAddResponse { Ids = new List<string> { "playarrtest" } });

            var remoteRom = CreateRemoteEpisode();
            remoteRom.Roms = Builder<Rom>.CreateListOfSize(1)
                                                      .All()
        ...
        public void should_test_develop_version_successfully()
        {
            ...
            var result = new PlayarrValidationResult(Subject.Test());
        ...
        {
            var path = @"C:\Test\Game.Title.S01E01".AsOsAgnostic();
        ...
        {
            var path = @"C:\Test\Game.Title.S01E01.mkv".AsOsAgnostic();
            _downloadClientItem.OutputPath = new OsPath(path);
