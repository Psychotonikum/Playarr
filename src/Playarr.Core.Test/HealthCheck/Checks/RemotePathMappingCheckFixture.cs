...
namespace Playarr.Core.Test.HealthCheck.Checks
{
    ...
    public class RemotePathMappingCheckFixture : CoreTest<RemotePathMappingCheck>
    {
        ...
        {
            GivenFolderExists(_downloadRootPath);
            var importEvent = new EpisodeImportedEvent(new LocalEpisode(), new RomFile(), new List<DeletedRomFile>(), true, new DownloadClientItem());
        ...
        public void should_return_permissions_error_on_episode_import_failed_event_if_file_exists()
        {
            ...
            GivenFileExists(localRom.Path);

            var importEvent = new EpisodeImportFailedEvent(new Exception(), localRom, true, new DownloadClientItem());
