...
{
    [V5ApiController]
    public class QueueController : RestControllerWithSignalR<QueueResource, Playarr.Core.Queue.Queue>,
                               IHandle<QueueUpdatedEvent>, IHandle<PendingReleasesUpdatedEvent>
    {
        ...
        public object RemoveMany([FromBody] QueueBulkResource resource, [FromQuery] string? message, [FromQuery] bool removeFromClient = true, [FromQuery] bool blocklist = false, [FromQuery] bool skipRedownload = false, [FromQuery] bool changeCategory = false)
        {
            ...
            var pendingToRemove = new List<Playarr.Core.Queue.Queue>();
        ...
        public PagingResource<QueueResource> GetQueue([FromQuery] PagingRequestResource paging, bool includeUnknownSeriesItems = true, [FromQuery] int[]? gameIds = null, DownloadProtocol? protocol = null, [FromQuery] int[]? languages = null, [FromQuery] int[]? quality = null, [FromQuery] QueueStatus[]? status = null, [FromQuery] QueueSubresource[]? includeSubresources = null)
        {
            var pagingResource = new PagingResource<QueueResource>(paging);
            var pagingSpec = pagingResource.MapToPagingSpec<QueueResource, Playarr.Core.Queue.Queue>(
            ...
            return pagingSpec.ApplyToPage((spec) => GetQueue(spec, gameIds?.ToHashSet() ?? [], protocol, languages?.ToHashSet() ?? [], quality?.ToHashSet() ?? [], status?.ToHashSet() ?? [], includeUnknownSeriesItems), (q) => MapToResource(q, includeSeries, includeEpisodes));
        }

        private PagingSpec<Playarr.Core.Queue.Queue> GetQueue(PagingSpec<Playarr.Core.Queue.Queue> pagingSpec, HashSet<int> gameIds, DownloadProtocol? protocol, HashSet<int> languages, HashSet<int> quality, HashSet<QueueStatus> status, bool includeUnknownSeriesItems)
        {
            var ascending = pagingSpec.SortDirection == SortDirection.Ascending;
            var orderByFunc = GetOrderByFunc(pagingSpec);
            var queue = _queueService.GetQueue();
            var filteredQueue = includeUnknownSeriesItems ? queue : queue.Where(q => q.Game != null);
            var pending = _pendingReleaseService.GetPendingQueue();
            var hasGameIdFilter = gameIds is { Count: > 0 };
            ...
            var fullQueue = filteredQueue.Concat(pending).Where(q =>
            {
                ...
                if (hasGameIdFilter)
                {
                    include &= q.Game != null && gameIds.Contains(q.Game.Id);
                }
            ...
            IOrderedEnumerable<Playarr.Core.Queue.Queue> ordered;
        ...
        private Func<Playarr.Core.Queue.Queue, object?> GetOrderByFunc(PagingSpec<Playarr.Core.Queue.Queue> pagingSpec)
        {
            switch (pagingSpec.SortKey)
            {
                ...
                case "game.sortTitle":
                    return q => q.Game?.SortTitle ?? q.Title;
                case "title":
                    return q => q.Title;
                case "rom":
                    return q => q.Roms.FirstOrDefault();
                case "rom.airDateUtc":
                case "roms.airDateUtc":
                    return q => q.Roms.FirstOrDefault()?.AirDateUtc ?? DateTime.MinValue;
                case "rom.title":
                case "roms.title":
                    return q => q.Roms.FirstOrDefault()?.Title ?? string.Empty;
                case "language":
                case "languages":
        ...
        private void Remove(Playarr.Core.Queue.Queue pendingRelease, string? message, bool blocklist)
        {
            if (blocklist)
            {
        ...
        private QueueResource MapToResource(Playarr.Core.Queue.Queue queueItem, bool includeSeries, bool includeEpisodes)
        {
