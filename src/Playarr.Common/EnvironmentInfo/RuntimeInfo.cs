...
namespace Playarr.Common.EnvironmentInfo
{
    public class RuntimeInfo : IRuntimeInfo
    {
        ...
        public RuntimeInfo(Logger logger, IOsInfo osInfo, IHostLifetime hostLifetime = null)
        {
            ...
            // net6.0 will return Playarr.dll for entry assembly, we need the actual
            // executable name (Playarr on linux).  On mono this will return the location of
            ...
            if (entry != null)
            {
                ...
                IsWindowsTray = OsInfo.IsWindows && entry.ModuleName == $"{ProcessProvider.PLAYARR_PROCESS_NAME}.exe";
            }
        }
        ...
        public bool IsTray
        {
            get
            {
                if (OsInfo.IsWindows)
                {
                    return IsUserInteractive && Process.GetCurrentProcess().ProcessName.Equals(ProcessProvider.PLAYARR_PROCESS_NAME, StringComparison.InvariantCultureIgnoreCase);
                }
