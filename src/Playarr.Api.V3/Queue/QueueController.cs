...
namespace Playarr.Api.V3.Queue
{
                               ...
                               IHandle<ObsoleteQueueUpdatedEvent>, IHandle<PendingReleasesUpdatedEvent>
    {
        ...
        public PagingResource<QueueResource> GetQueue([FromQuery] PagingRequestResource paging, bool includeUnknownSeriesItems = false, bool includeSeries = false, bool includeEpisode = false, [FromQuery] int[] gameIds = null, DownloadProtocol? protocol = null, [FromQuery] int[] languages = null, [FromQuery] int[] quality = null, [FromQuery] QueueStatus[] status = null)
        {
            ...
            return pagingSpec.ApplyToPage((spec) => GetQueue(spec, gameIds?.ToHashSet(), protocol, languages?.ToHashSet(), quality?.ToHashSet(), status?.ToHashSet(), includeUnknownSeriesItems), (q) => MapToResource(q, includeSeries, includeEpisode));
        }
        ...
        private Func<Playarr.Core.Queue.Queue, object> GetOrderByFunc(PagingSpec<Playarr.Core.Queue.Queue> pagingSpec)
        {
            switch (pagingSpec.SortKey)
            {
                ...
                case "roms.airDateUtc":
                    return q => q.Rom?.AirDateUtc ?? DateTime.MinValue;
                case "rom.title":
                case "roms.title":
                    return q => q.Rom?.Title ?? string.Empty;
                case "language":
                case "languages":
        ...
        private void Remove(Playarr.Core.Queue.Queue pendingRelease, bool blocklist)
        {
            if (blocklist)
            {
        ...
        private QueueResource MapToResource(Playarr.Core.Queue.Queue queueItem, bool includeSeries, bool includeEpisode)
        {
