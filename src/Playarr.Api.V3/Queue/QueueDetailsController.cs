...
namespace Playarr.Api.V3.Queue
{
                               ...
                               IHandle<ObsoleteQueueUpdatedEvent>, IHandle<PendingReleasesUpdatedEvent>
    {
        ...
        public List<QueueResource> GetQueue(int? gameId, [FromQuery]List<int> romIds, bool includeSeries = false, bool includeEpisode = false)
        {
            ...
            if (gameId.HasValue)
            {
                return fullQueue.Where(q => q.Game?.Id == gameId).ToResource(includeSeries, includeEpisode);
            }

            if (romIds.Any())
            {
                return fullQueue.Where(q => q.Rom != null && romIds.Contains(q.Rom.Id)).ToResource(includeSeries, includeEpisode);
            }
