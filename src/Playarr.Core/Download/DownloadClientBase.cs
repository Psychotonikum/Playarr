...
namespace Playarr.Core.Download
{
        ...
        where TSettings : IProviderConfig, new()
    {
        ...
        protected ValidationFailure TestFolder(string folder, string propertyName, bool mustBeWritable = true)
        {
            ...
            {
                return new PlayarrValidationFailure(propertyName, "Folder does not exist")
                {
                    DetailedDescription = string.Format("The folder you specified does not exist or is inaccessible. Please verify the folder permissions for the user account '{0}', which is used to execute Playarr.", Environment.UserName)
                };
            }
            ...
            if (mustBeWritable && !_diskProvider.FolderWritable(folder))
            {
                ...
                return new PlayarrValidationFailure(propertyName, "Unable to write to folder")
                {
                    DetailedDescription = string.Format("The folder you specified is not writable. Please verify the folder permissions for the user account '{0}', which is used to execute Playarr.", Environment.UserName)
                };
            }
