...
namespace Playarr.Core.Download
{
    ...
    public class ProcessDownloadDecisions : IProcessDownloadDecisions
    {
        ...
        {
            // If a release was already grabbed with matching roms we should store it as a fallback
        ...
        private async Task<ProcessedDecisionResult> ProcessDecisionInternal(DownloadDecision decision, int? downloadClientId = null)
        {
            ...
            try
            {
                _logger.Trace("Grabbing release '{0}' from Indexer {1} at priority {2}.", remoteRom, remoteIndexer, remoteRom.Release.IndexerPriority);
                await _downloadService.DownloadReport(remoteRom, downloadClientId);

                return ProcessedDecisionResult.Grabbed;
            }
            catch (ReleaseUnavailableException)
            {
                _logger.Warn("Failed to download release '{0}' from Indexer {1}. Release not available", remoteRom, remoteIndexer);
                return ProcessedDecisionResult.Rejected;
            }
            catch (Exception ex)
            {
                ...
                {
                    _logger.Debug(ex, "Failed to send release '{0}' from Indexer {1} to download client, storing until later.", remoteRom, remoteIndexer);
                    return ProcessedDecisionResult.Failed;
                }
                else
                {
                    _logger.Warn(ex, "Couldn't add release '{0}' from Indexer {1} to download queue.", remoteRom, remoteIndexer);
                    return ProcessedDecisionResult.Skipped;
                }
            }
        }
