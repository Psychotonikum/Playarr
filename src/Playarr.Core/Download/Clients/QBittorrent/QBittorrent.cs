...
using Playarr.Core.MediaFiles.TorrentInfo;
    ...
    public class QBittorrent : TorrentClientBase<QBittorrentSettings>
    {
        ...
        public override void MarkItemAsImported(DownloadClientItem downloadClientItem)
        {
            ...
            if (Settings.TvImportedCategory.IsNotNullOrWhiteSpace() &&
                Settings.TvImportedCategory != Settings.GameCategory)
            {
                try
                {
        ...
        protected override string AddFromMagnetLink(RemoteRom remoteRom, string hash, string magnetLink)
        {
            ...
            var isRecentEpisode = remoteRom.IsRecentEpisode();
            ...
            Proxy.AddTorrentFromUrl(magnetLink, addHasSetShareLimits && setShareLimits ? remoteRom.SeedConfiguration : null, Settings);

            if ((!addHasSetShareLimits && setShareLimits) || moveToTop || forceStart || (Settings.AddGameTags && remoteRom.Game.Tags.Count > 0))
            {
                ...
                if (!addHasSetShareLimits && setShareLimits)
                {
                    try
                    {
                        Proxy.SetTorrentSeedingConfiguration(hash.ToLower(), remoteRom.SeedConfiguration, Settings);
                    }
                    catch (Exception ex)
                    {
                ...
                if (Settings.AddGameTags && remoteRom.Game.Tags.Count > 0)
                {
                    try
                    {
                        Proxy.AddTags(hash.ToLower(), _tagRepository.GetTags(remoteRom.Game.Tags).Select(tag => tag.Label), Settings);
                    }
                    catch (Exception ex)
                    {
        ...
        protected override string AddFromTorrentFile(RemoteRom remoteRom, string hash, string filename, byte[] fileContent)
        {
            ...
            Proxy.AddTorrentFromFile(filename, fileContent, addHasSetShareLimits ? remoteRom.SeedConfiguration : null, Settings);
        ...
        private ValidationFailure TestConnection()
        {
            try
            {
                ...
                if (version < Version.Parse("1.5"))
                {
                    ...
                    return new PlayarrValidationFailure("Host", _localizationService.GetLocalizedString("DownloadClientValidationErrorVersion",
                ...
                else if (version < Version.Parse("1.6"))
                {
                    ...
                    {
                        return new PlayarrValidationFailure("Category", _localizationService.GetLocalizedString("DownloadClientQbittorrentValidationCategoryUnsupported"))
                        {
                ...
                else if (Settings.GameCategory.IsNullOrWhiteSpace())
                {
                    ...
                    return new PlayarrValidationFailure("GameCategory", _localizationService.GetLocalizedString("DownloadClientQbittorrentValidationCategoryRecommended"))
                    {
                        IsWarning = true,
                ...
                {
                    return new PlayarrValidationFailure(string.Empty, _localizationService.GetLocalizedString("DownloadClientQbittorrentValidationRemovesAtRatioLimit"))
                    {
            ...
            catch (DownloadClientAuthenticationException ex)
            {
                ...
                return new PlayarrValidationFailure(Settings.ApiKey.IsNotNullOrWhiteSpace() ? "ApiKey" : "Username", _localizationService.GetLocalizedString("DownloadClientValidationAuthenticationFailure"))
                {
            ...
            catch (WebException ex)
            {
                ...
                {
                    return new PlayarrValidationFailure("Host", _localizationService.GetLocalizedString("DownloadClientValidationUnableToConnect", new Dictionary<string, object> { { "clientName", Name } }))
                    {
                        DetailedDescription = _localizationService.GetLocalizedString("DownloadClientValidationUnableToConnectDetail")
                    };
                }

                return new PlayarrValidationFailure(string.Empty, _localizationService.GetLocalizedString("DownloadClientValidationUnknownException", new Dictionary<string, object> { { "exception", ex.Message } }));
            }
            catch (Exception ex)
            {
        ...
        {
            if (Settings.GameCategory.IsNullOrWhiteSpace() && Settings.TvImportedCategory.IsNullOrWhiteSpace())
            {
                return null;
            }
            ...
            if (Settings.GameCategory.IsNotNullOrWhiteSpace() && !labels.ContainsKey(Settings.GameCategory))
            {
                ...
                {
                    return new PlayarrValidationFailure("GameCategory", _localizationService.GetLocalizedString("DownloadClientQbittorrentValidationCategoryAddFailure"))
                    {
            ...
            if (Settings.TvImportedCategory.IsNotNullOrWhiteSpace() && !labels.ContainsKey(Settings.TvImportedCategory))
            {
                ...
                {
                    return new PlayarrValidationFailure("TvImportedCategory", _localizationService.GetLocalizedString("DownloadClientQbittorrentValidationCategoryAddFailure"))
                    {
                        DetailedDescription = _localizationService.GetLocalizedString("DownloadClientQbittorrentValidationCategoryAddFailureDetail")
                    };
                }
        ...
        private ValidationFailure TestPrioritySupport()
        {
            ...
            try
            {
                ...
                if (!config.QueueingEnabled)
                {
                    ...
                    {
                        return new PlayarrValidationFailure(nameof(Settings.RecentTvPriority), _localizationService.GetLocalizedString("DownloadClientQbittorrentValidationQueueingNotEnabled"))
                        {
                    ...
                    {
                        return new PlayarrValidationFailure(nameof(Settings.OlderTvPriority), _localizationService.GetLocalizedString("DownloadClientQbittorrentValidationQueueingNotEnabled"))
                        {
                            DetailedDescription = _localizationService.GetLocalizedString("DownloadClientQbittorrentValidationQueueingNotEnabledDetail")
                        };
                    }
        ...
        private ValidationFailure TestGetTorrents()
        {
            ...
            catch (Exception ex)
            {
                ...
                return new PlayarrValidationFailure(string.Empty, _localizationService.GetLocalizedString("DownloadClientValidationTestTorrents", new Dictionary<string, object> { { "exceptionMessage", ex.Message } }));
            }
