...
namespace Playarr.Core.Download.Clients.NzbVortex
{
    public class NzbVortex : UsenetClientBase<NzbVortexSettings>
    {
        ...
        {
            var group = GetGroups().FirstOrDefault(c => c.GroupName == Settings.GameCategory);

            if (group == null)
            {
                if (Settings.GameCategory.IsNotNullOrWhiteSpace())
                {
                    return new PlayarrValidationFailure("GameCategory", _localizationService.GetLocalizedString("DownloadClientValidationGroupMissing"))
                    {
