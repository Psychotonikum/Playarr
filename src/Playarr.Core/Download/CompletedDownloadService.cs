...
namespace Playarr.Core.Download
{
    ...
    public class CompletedDownloadService : ICompletedDownloadService
    {
        ...
        public void Check(TrackedDownload trackedDownload)
        {
            ...
            {
                trackedDownload.Warn("Download wasn't grabbed by Playarr and not in a category, Skipping.");
                return;
            }
            ...
            if (game == null)
            {
                ...
                if (game == null)
                {
                    trackedDownload.Warn("Game title mismatch; automatic import is not possible. Check the download troubleshooting entry on the wiki for common causes.");
                ...
                {
                    trackedDownload.Warn("Found matching game via grab history, but release was matched to game by ID. Automatic import is not possible. See the FAQ for details.");
                    SetStateToImportBlocked(trackedDownload);
        ...
        public void Import(TrackedDownload trackedDownload)
        {
            ...
            var statusMessages = new List<TrackedDownloadStatusMessage>
                                 {
                                    new TrackedDownloadStatusMessage("One or more roms expected in this release were not imported or missing from the release", new List<string>())
                                 };
        ...
        public bool VerifyImport(TrackedDownload trackedDownload, List<ImportResult> importResults)
        {
            ...
            {
                _logger.Debug("All roms were imported for {0}", trackedDownload.DownloadItem.Title);
            ...
            // Double check if all roms were imported by checking the history if at least one
            // file was imported. This will allow the decision engine to reject already imported
            // rom files and still mark the download complete when all files are imported.

            // EDGE CASE: This process relies on RomIds being consistent between executions, if a game is updated
            // and an rom is removed, but later comes back with a different ID then Playarr will treat it as incomplete.
            ...
            if (allEpisodesImportedInHistory)
            {
                ...
                {
                    _logger.Debug("All roms were imported in history for {0}", trackedDownload.DownloadItem.Title);
                }
                else
                {
            ...
            _logger.Debug("Not all roms have been imported for the release '{0}'", trackedDownload.DownloadItem.Title);
            return false;
        }
