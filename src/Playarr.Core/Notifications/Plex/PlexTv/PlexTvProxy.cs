...
namespace Playarr.Core.Notifications.Plex.PlexTv
{
    ...
    public class PlexTvProxy : IPlexTvProxy
    {
        ...
        private string ProcessRequest(HttpRequestBuilder requestBuilder)
        {
            ...
            {
                throw new PlayarrClientException(ex.Response.StatusCode, "Unable to connect to plex.tv");
            }
            catch (WebException)
            {
                throw new PlayarrClientException(HttpStatusCode.BadRequest, "Unable to connect to plex.tv");
            }
