...
namespace Playarr.Core.Notifications.Emby
{
    ...
    public class MediaBrowserService : IMediaBrowserService
    {
        ...
        public void Update(MediaBrowserSettings settings, Game game, string updateType)
        {
            ...
            paths = _proxy.GetPaths(settings, game);
        ...
        public ValidationFailure Test(MediaBrowserSettings settings)
        {
            ...
            if (settings.Notify)
            {
                try
                {
                    Notify(settings, "Test from Playarr", "Success! MediaBrowser has been successfully configured!");
                }
                catch (HttpException ex)
                {
