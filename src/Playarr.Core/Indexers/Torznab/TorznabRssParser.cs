...
namespace Playarr.Core.Indexers.Torznab
{
    public class TorznabRssParser : TorrentRssParser
    {
        ...
        protected virtual int GetMobyGamesId(XElement item)
        {
            var mobyGamesIdString = TryGetTorznabAttribute(item, "rageid");

            if (!mobyGamesIdString.IsNullOrWhiteSpace() && int.TryParse(mobyGamesIdString, out var mobyGamesId))
            {
                return mobyGamesId;
            }
