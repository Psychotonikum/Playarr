...
namespace Playarr.Core.Datastore.Migration.Framework
{
    public static class MigrationExtension
    {
        ...
        public static IMigrationRunnerBuilder AddPlayarrSQLite(this IMigrationRunnerBuilder builder, bool binaryGuid = false, bool useStrictTables = false)
        {
            builder.Services
                .AddTransient<SQLiteBatchParser>()
                .AddScoped<SQLiteDbFactory>()
                .AddScoped<PlayarrSQLiteProcessor>(sp =>
                {
                    var factory = sp.GetService<SQLiteDbFactory>();
                    var logger = sp.GetService<ILogger<PlayarrSQLiteProcessor>>();
                    ...
                    return new PlayarrSQLiteProcessor(factory, sp.GetService<SQLiteGenerator>(), logger, options, connectionStringAccessor, sp, sqliteQuoter);
                })
                .AddScoped<ISQLiteTypeMap>(_ => new PlayarrSQLiteTypeMap(useStrictTables))
                .AddScoped<IMigrationProcessor>(sp => sp.GetRequiredService<PlayarrSQLiteProcessor>())
                .AddScoped(
                    sp =>
                    {
