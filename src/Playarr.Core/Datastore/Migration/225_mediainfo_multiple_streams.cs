...
[Migration(225)]
public class mediainfo_multiple_streams : PlayarrMigrationBase
{
    ...
    {
        var existing = conn.Query<RomFile224>("SELECT \"Id\", \"MediaInfo\" FROM \"EpisodeFiles\"");
        ...
        foreach (var row in existing)
        {
            ...
            {
                _logger.Warn(ex, "Rom {EpisodeId} contains invalid JSON data, skipping.", row.Id);

                updated.Add(new RomFile225 { Id = row.Id, MediaInfo = null });
            ...
            {
                _logger.Warn("Rom {EpisodeId} might be already migrated, skipping.", row.Id);

                continue;
            }

            updated.Add(new RomFile225
            {
                Id = row.Id,
    ...
    internal class RomFile224 : ModelBase
    {
        public string MediaInfo { get; set; }
        public string SceneName { get; set; }
    }

    internal class RomFile225 : ModelBase
    {
        public string MediaInfo { get; set; }
    }
