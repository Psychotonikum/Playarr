...
{
    [Migration(81)]
    public class move_dot_prefix_to_transmission_category : PlayarrMigrationBase
    {
            ...
            using (var cmd = conn.CreateCommand())
            {
                ...
                using (var reader = cmd.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        ...
                        var gameCategory = settings.GetValueOrDefault("gameCategory") as string;
                        if (gameCategory.IsNotNullOrWhiteSpace())
                        {
                            settings["gameCategory"] = "." + gameCategory;
