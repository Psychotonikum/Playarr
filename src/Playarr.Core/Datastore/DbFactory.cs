...
namespace Playarr.Core.Datastore
{
    ...
    {
        private static readonly Logger Logger = PlayarrLogger.GetLogger(typeof(DbFactory));
        ...
        private void CreateMain(string connectionString, MigrationContext migrationContext, DatabaseType databaseType)
        {
            ...
            catch (SQLiteException e)
            {
                ...
                if (OsInfo.IsOsx)
                {
                    throw new CorruptDatabaseException("Database file: {0} is corrupt, restore from backup if available. See: https://wiki.servarr.com/playarr/faq#i-use-playarr-on-a-mac-and-it-suddenly-stopped-working-what-happened", e, fileName);
                }

                throw new CorruptDatabaseException("Database file: {0} is corrupt, restore from backup if available. See: https://wiki.servarr.com/playarr/faq#i-am-getting-an-error-database-disk-image-is-malformed", e, fileName);
            }
                ...
                if (e.InnerException is SocketException)
                {
                    ...
                    while (true)
                    {
                        ...
                        catch (Exception ex)
                        {
                            ...
                            throw new PlayarrStartupException(ex, "Error creating main database");
                        }
                    }
                }
                else
                {
                    throw new PlayarrStartupException(e, "Error creating main database");
                }
            }
            catch (Exception e)
            {
        ...
        private void CreateLog(string connectionString, MigrationContext migrationContext, DatabaseType databaseType)
        {
            ...
            {
                throw new PlayarrStartupException(e, "Error creating log database");
            }
        }
    }
}
