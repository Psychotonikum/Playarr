...
{
    public class SearchSeriesComparer : IComparer<Game>
    {
        ...
        public int Compare<T>(Game x, Game y, Func<Game, T> keySelector)
        ...
        public int CompareWithYear(Game x, Game y, Predicate<Game> canMatch)
        {
        ...
        private int GetYearFactor(Game game)
        {
            if (_year.HasValue)
            {
                var offset = Math.Abs(game.Year - _year.Value);
                if (offset <= 1)
                {
                    return 20 - (10 * offset);
                }
