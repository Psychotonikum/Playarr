...
namespace Playarr.Test.Common
{
    public class PlayarrRunner
    {
        ...
        public PlayarrRunner(Logger logger, PostgresOptions postgresOptions, int port = 9797)
        {
        ...
        public void Start(bool enableAuth = false)
        {
            ...
            if (OsInfo.IsWindows)
            {
                consoleExe = "Playarr.Console.exe";
            }
            else
            {
                consoleExe = "Playarr";
            ...
            while (true)
            {
                ...
                {
                    TestContext.Progress.WriteLine($"Playarr {Port} is started. Running Tests");
                    return;
                }

                TestContext.Progress.WriteLine("Waiting for Playarr to start. Response Status : {0}  [{1}] {2}", statusCall.ResponseStatus, statusCall.StatusDescription, statusCall.ErrorException.Message);
        ...
        public void Kill()
        {
            try
            {
                if (_nzbDroneProcess != null)
                {
                    ...
                    if (_nzbDroneProcess.HasExited)
                    {
                        var log = File.ReadAllLines(Path.Combine(AppData, "logs", "Playarr.trace.txt"));
        ...
        private void Start(string outputPlayarrConsoleExe)
        {
            StringDictionary envVars = new();
            if (PostgresOptions?.Host != null)
            {
                envVars.Add("Playarr__Postgres__Host", PostgresOptions.Host);
                envVars.Add("Playarr__Postgres__Port", PostgresOptions.Port.ToString());
                envVars.Add("Playarr__Postgres__User", PostgresOptions.User);
                envVars.Add("Playarr__Postgres__Password", PostgresOptions.Password);
                envVars.Add("Playarr__Postgres__MainDb", PostgresOptions.MainDb);
                envVars.Add("Playarr__Postgres__LogDb", PostgresOptions.LogDb);

                TestContext.Progress.WriteLine("Using env vars:\n{0}", envVars.ToJson());
            }

            TestContext.Progress.WriteLine("Starting instance from {0} on port {1}", outputPlayarrConsoleExe, Port);

            var args = "-nobrowser -nosingleinstancecheck -data=\"" + AppData + "\"";
            _nzbDroneProcess = _processProvider.Start(outputPlayarrConsoleExe, args, envVars, OnOutputDataReceived, OnOutputDataReceived);
        }
