...
import styles from './GameSearchInput.css';
...
interface AddNewGameSuggestion {
  type: 'addNew';
  title: string;
}
...
interface Section {
  title: string;
  loading?: boolean;
  suggestions: SeriesSuggestion[] | AddNewGameSuggestion[];
}

function useGameSuggestions(tagList: Tag[]) {
  const { data: allGames = [] } = useGame();

  return useMemo(() => {
    return allGames.map((game): SuggestedSeries => {
      const {
        title,
        titleSlug,
        sortTitle,
...
function GameSearchInput() {
  ...
  const game = useGameSuggestions(tagList);
  ...
  const renderSuggestion = useCallback(
    (
      item: AddNewGameSuggestion | SeriesSuggestion,
      { query }: { query: string }
    ) => {
      if ('type' in item) {
        return (
          <div className={styles.addNewGameSuggestion}>
  ...
  const handleKeyDown = useCallback(
    (event: KeyboardEvent<HTMLElement>) => {
      ...
      // If a suggestion is not selected go to the first game,
      // otherwise go to the selected game.
      ...
      dispatch(
        push(
          `${window.Playarr.urlBase}/game/${selectedSuggestion.item.titleSlug}`
        )
      );
  ...
  const handleSuggestionSelected = useCallback(
    (
      ...
      { suggestion }: { suggestion: SeriesSuggestion | AddNewGameSuggestion }
    ) => {
      if ('type' in suggestion) {
        ...
        setValue('');
        dispatch(
          push(`${window.Playarr.urlBase}/game/${suggestion.item.titleSlug}`)
        );
      }
    },
    [value, dispatch]
  ...
  useEffect(() => {
    ...
    return () => {
      unbindShortcut('focusGameSearchInput');
    };
